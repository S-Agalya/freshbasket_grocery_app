...
import { FaBars, FaTachometerAlt, FaBoxOpen, FaSignOutAlt } from "react-icons/fa";
import axios from "axios";
...
function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Mobile Header */}
      <header className="lg:hidden flex items-center justify-between bg-green-700 text-white px-4 py-3 shadow-md fixed top-0 left-0 right-0 z-50">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen w-64 bg-green-700 text-white flex flex-col justify-between shadow-lg transform transition-transform duration-300 z-40
        ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} 
        lg:translate-x-0`}
      >
        <div>
          <h1 className="hidden lg:block text-2xl font-bold text-center p-6 border-b border-green-800">
            Admin Panel
          </h1>
          <nav className="flex flex-col p-4 space-y-2">
            {menuItems.map((item) => (
      ...
      {/* Main Content */}
      <main
        className="flex-1 ml-0 lg:ml-64 mt-14 lg:mt-0 p-6 overflow-auto h-screen"
